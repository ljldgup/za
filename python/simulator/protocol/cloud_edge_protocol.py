...
# from memory_profiler import profile as m_profile
...
def publish_mqtt_msg(client, msg, topic):
    if common_config["log"]["log_device_message"]:
        simulator_logger.info("publish to {}  {}".format(topic, msg))
    # client.publish(topic=topic, payload=json.dumps(msg), qos=0, retain=False)
    client.publish(topic=topic, payload=json.dumps(msg), qos=0, retain=False)
...
def publish_service_data_upload_message(client, device_id):
    ...
    if 'data' in model_info:
        ...
        topic = '/{}/{}/service/data'.format(version, client.gateway_id)
        publish_mqtt_msg(client, response, topic)
    # except:
    #     traceback.print_exc()
...
def schedule_publisher(clients):
    # 不知为什么入参一直不对，写成args=(client,)也不对，只能封装成lambda
    threading.Thread(target=lambda: schedule_publish(clients), ).start()
...
def direct_publish(clients, functions):
    try:
        for func in functions:
            for client in clients:
                for device_id in client.gateway_children_map.keys():
...
def async_direct_publish(clients):
    # 不知为什么入参一直不对，写成args=(client,)也不对，只能封装成lambda
    direct_publish(clients, wrapped_functions, )


# @m_profile(precision=4)
def profile_test():
    clt = type('temp', (object,), {})()
    clt.gateway_id = '1574696641356816384'
    clt.publish = lambda **kwargs: 1
    clt.device_config = device_config.device_config
    for i in range(100000):
        for func in [publish_service_event_upload_message, publish_service_data_upload_message]:
            func(clt, '1574696641356816384')


def publish_test(clients):
    for func in [publish_service_event_upload_message, publish_service_data_upload_message]:
        for client in clients:
            for device_id in client.gateway_children_map.keys():
                func(client, device_id)
